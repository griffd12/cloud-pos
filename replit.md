...
- **Concurrency-Safe Check Numbering**: Check numbers are generated using a dedicated `rvc_counters` table with atomic `INSERT...ON CONFLICT DO UPDATE...RETURNING` inside a database transaction. The `createCheckAtomic()` method wraps counter reservation + check insert in a single transaction — if the insert fails, the counter rolls back (no gaps). A `UNIQUE(rvc_id, check_number)` constraint on the `checks` table acts as a safety net. Counters are RVC-scoped and auto-created on first use via UPSERT. All check creation paths (POS, split check, delivery platform injection) use `createCheckAtomic()`. Tested: 10 concurrent requests produce 10 unique sequential numbers, zero duplicates.
